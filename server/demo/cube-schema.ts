...
export const employeesCube = defineCube("Employees", {
	...
	dimensions: {
		...
		age: {
			sql: "DATEDIFF('year', e.birth_date, CURRENT_DATE)",
			type: "number",
			title: "Age",
		},
	...
	measures: {
		...
		activePercentage: {
			type: "calculated",
			calculatedSql:
				"({Employees.activeCount} / NULLIF({Employees.count}, 0)) * 100",
		},
		avgSalaryPerActive: {
			type: "calculated",
			calculatedSql:
				"{Employees.totalSalary} / NULLIF({Employees.activeCount}, 0)",
		},
	},
	joins: {
		Departments: {
...
export const departmentsCube = defineCube("Departments", {
	...
	measures: {
		...
		budgetPerDepartment: {
			type: "calculated",
			calculatedSql:
				"{Departments.totalBudget} / NULLIF({Departments.count}, 0)",
		},
	},
	joins: {
		Employees: {
...
export const productivityCube = defineCube("Productivity", {
	...
	measures: {
		...
		workingDaysPercentage: {
			type: "calculated",
			calculatedSql:
				"({Productivity.workingDaysCount} / NULLIF({Productivity.recordCount}, 0)) * 100",
		},
		avgCodePerWorkday: {
			type: "calculated",
			calculatedSql:
				"{Productivity.totalLinesOfCode} / NULLIF({Productivity.workingDaysCount}, 0)",
		},
		avgPRsPerWorkday: {
			type: "calculated",
			calculatedSql:
				"{Productivity.totalPullRequests} / NULLIF({Productivity.workingDaysCount}, 0)",
		},
		compositeProductivityScore: {
			type: "calculated",
			calculatedSql:
				"({Productivity.totalLinesOfCode} * 0.3 + {Productivity.totalPullRequests} * 2 + {Productivity.totalDeployments} * 5) / NULLIF({Productivity.workingDaysCount}, 0)",
		},
		deploymentRate: {
			type: "calculated",
			calculatedSql:
				"({Productivity.totalDeployments} / NULLIF({Productivity.workingDaysCount}, 0)) * 100",
		},
		// Window functions
		linesOfCodeChange: {
			sql: "p.lines_of_code",
...
export const SAMPLES: { name: string; json: Record<string, unknown> }[] = [
	{
		...
		json: {
			...
			dimensions: ["Employees.band", "Employees.country"],
		},
	},
	{
	...
	},
	{
		...
		json: {
			...
			dimensions: ["Employees.nationality"],
		},
	},
	{
	...
	},
	{
		...
		json: {
			...
			operation: "update",
			values: { band: "E3" },
			filters: [{ member: "id", operator: "equals", values: ["1"] }],
		},
	},
	{
		...
		json: {
			cube: "Employees",
			operation: "delete",
			filters: [{ member: "id", operator: "equals", values: ["250"] }],
		},
	},
];
